...
if not os.environ.get("SRC_MONGO_URL"):
    print("please set [SRC_MONGO_URL]")
    exit(1)

if not os.environ.get("DST_MONGO_URL"):
    print("please set [DST_MONGO_URL]")
    exit(1)

db = pymongo.MongoClient(
    os.environ.get("SRC_MONGO_URL"),
    socketTimeoutMS=2000,
    connectTimeoutMS=1000,
    serverSelectionTimeoutMS=1000,
    connect=True,
)
print(db["local"]["oplog.rs"])
...
full_sync.http_port = {FULL_SYNC_HTTP_PORT}
incr_sync.http_port = {INCR_SYNC_HTTP_PORT}
# profiling on net/http/profile
# profiling端口，用于查看内部go堆栈。
system_profile_port = {SYSTEM_PROFILE_PORT}
...
mongo_urls = {SRC_MONGO_URL or MONGO_URLS}
...
tunnel.address = {DST_MONGO_URL or TUNNEL_ADDRESS}
...
tunnel.kafka.partition_number = {TUNNEL_KAFKA_PARTITION_NUMBER}
...
mongo_connect_mode = {MONGO_CONNECT_MODE}
...
# checkpoint存储的db的名字
checkpoint.storage.db = {CHECKPOINT_STORAGE_DB}
# checkpoint collection's name.
# checkpoint存储的表的名字，如果启动多个mongoshake拉取同一个源可以修改这个表名以防止冲突。
checkpoint.storage.collection = {CHECKPOINT_STORAGE_COLLECTION}
# set if enable ssl
...
# 大于给定的时间，如果是则会直接报错退出。
checkpoint.start_position = {CHECKPOINT_START_POSITION}
...
full_sync.reader.collection_parallel = {FULL_SYNC_READER_COLLECTION_PARALLELS}
# the number of document writer thread in each collection.
# 同一个表内并发写的线程数，例如，8表示对于同一个表，将会有8个写线程进行并发写入。
full_sync.reader.write_document_parallel = {FULL_SYNC_READER_WRITE_DOCUMENT_PARALLEL}
# number of documents in a batch insert in a document concurrence
# 目的端写入的batch大小，例如，128表示一个线程将会一次聚合128个文档然后再写入。
full_sync.reader.document_batch_size = {FULL_SYNC_READER_DOCUMENT_BATCH_SIZE}
# number of documents in a batch in fetch from source db
# 源端拉取batch中最大条数
full_sync.reader.fetch_batch_size = {FULL_SYNC_READER_FETCH_BATCH_SIZE}
...
full_sync.reader.parallel_thread = {FULL_SYNC_READER_PARALLEL_THREAD}
...
full_sync.reader.parallel_index = {FULL_SYNC_READER_PARALLEL_INDEX}

# drop the same name of collection in dest mongodb in full synchronization
# 同步时如果目的库存在，是否先删除目的库再进行同步，true表示先删除再同步，false表示不删除。
full_sync.collection_exist_drop = {FULL_SYNC_COLLECTION_EXIST_DROP}
...
# background表示创建后台索引。
full_sync.create_index = {FULL_SYNC_CREATE_INDEX}

# convert insert to update when duplicate key found
# 如果_id存在在目的库，是否将insert语句修改为update语句。
full_sync.executor.insert_on_dup_update = {FULL_SYNC_EXECUTOR_INSERT_ON_DUP_UPDATE}
# filter orphan document for source type is sharding.
# 源端是sharding，是否需要过滤orphan文档
full_sync.executor.filter.orphan_document = {FULL_SYNC_EXECUTOR_FILTER_ORPHAN_DOCUMENT}
# enable majority write in full sync.
# the performance will degrade if enable.
# 全量阶段写入端是否启用majority write
full_sync.executor.majority_enable = {FULL_SYNC_EXECUTOR_MAJORITY_ENABLE}
...
incr_sync.mongo_fetch_method = {INCR_SYNC_MONGO_FETCH_METHOD}
...
incr_sync.change_stream.watch_full_document = {INCR_SYNC_CHANGE_STREAM_WATCH_FULL_DOCUMENT}
...
incr_sync.shard_key = {INCR_SYNC_SHARD_KEY}
...
incr_sync.worker = {INCR_SYNC_WORKER}
...
incr_sync.tunnel.write_thread = {INCR_SYNC_TUNNEL_WRITE_THREAD}
...
# 0表示不启用
incr_sync.target_delay = {INCR_SYNC_TARGET_DELAY}
...
incr_sync.worker.batch_queue_size = {INCR_SYNC_WORKER_BATCH_QUEUE_SIZE}
incr_sync.adaptive.batching_max_size = {INCR_SYNC_ADAPTIVE_BATCHING_MAX_SIZE}
incr_sync.fetcher.buffer_capacity = {INCR_SYNC_FETCHER_BUFFER_CAPACITY}
incr_sync.reader.fetch_batch_size = {INCR_SYNC_READER_FETCH_BATCH_SIZE}
...
incr_sync.executor.upsert = {INCR_SYNC_EXECUTOR_UPSERT}
# oplog changes to Update while Insert found duplicated key (_id or unique-index)
# 如果_id存在在目的库，是否将insert语句修改为update语句。
incr_sync.executor.insert_on_dup_update = {INCR_SYNC_EXECUTOR_INSERT_ON_DUP_UPDATE}
# options:db,none
# db. write duplicated logs to mongoshake_conflict
# 如果写入存在冲突，记录冲突的文档。选项：db, none
# db：冲突写到目的DB的mongshake_conflict库中
incr_sync.conflict_write_to = {INCR_SYNC_CONFLICT_WRITE_TO}

# enable majority write in incrmental sync.
# the performance will degrade if enable.
# 增量阶段写入端是否启用majority write
incr_sync.executor.majority_enable = {INCR_SYNC_EXECUTOR_MAJORITY_ENABLE}
...
"""
with open("collector.conf.cur",mode="w") as fout:
    fout.write(config)